...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    try {
        ...
        if (!/audio/.test(mime)) throw m.reply(`Balas vn/audio yang ingin diubah dengan caption *${usedPrefix + command}*`)
        let audio = await q.download()
        if (!audio) throw m.reply('Can\'t download audio!')
        let set
        ...
        let media = path.join(__dirname, '../src/' + ran)
...
handler.help = ['audio8d', 'bass', 'blown', 'deep', 'earrape', 'fast', 'fat', 'nightcore', 'reverse', 'robot', 'slow', 'smooth', 'tupai', 'vibra'].map(v => v + ' [vn]')
handler.tags = ['Pengubah Suara']
handler.command = /^(audio8d|bass|blown|deep|earrape|fas?t|nightcore|reverse|robot|slow|smooth|tupai|squirrel|chipmunk|vibra)$/i
