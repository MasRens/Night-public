...
handler.before = async function(m) {
	var id = m.chat
	if (!m.quoted || !m.text || !m.quoted.fromMe || !m.quoted.isBaileys || !/Ketik.*who|hint/i.test(m.quoted.text)) return !0
	...
	if (m.quoted.id == this.siapakahaku[id][0].id) {
		...
		if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
			...
			db.data.users[m.sender].money += this.siapakahaku[id][2]
			conn.sendButton(m.chat, `*Benar!*\n+${this.siapakahaku[id][2]} Exp & Money`, author, ['siapakahaku', '/siapakahaku'], m)
