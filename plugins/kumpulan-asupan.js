...
case 'asupan':
          ...
          conn.sendVideo(m.chat, pickRandom(asupan), `${command}`, m)
          break
case 'asupanloli':
          conn.sendMessage(m.chat, {react: {text: ('🕟'), key: m.key, }})
          conn.sendVideo(m.chat, pickRandom(asupanloli), `${command}`, m)
          break
case 'asupanbocil':
           res = await fetch('https://raw.githubusercontent.com/binjaicity/warga62/master/bocil.json')
           asup = await res.json()
           conn.sendMessage(m.chat, {react: {text: ('🕟'), key: m.key, }})
           json = asup[Math.floor(Math.random() * asup.length)]
           conn.sendVideo(m.chat, json.url, `Asupan Bocil\n\n${mess.done}`,m)
           break
case 'asupangeayubi':
           res = await fetch('https://raw.githubusercontent.com/binjaicity/warga62/master/geayubi.json')
           asup = await res.json()
           conn.sendMessage(m.chat, {react: {text: ('🕟'), key: m.key, }})
           json = asup[Math.floor(Math.random() * asup.length)]
           conn.sendVideo(m.chat, json.url, `Asupan Geayubi\n\n${mess.done}`,m)
           break
case 'asupansantuy':
           res = await fetch('https://raw.githubusercontent.com/binjaicity/warga62/master/santuy.json')
           asup = await res.json()
           conn.sendMessage(m.chat, {react: {text: ('🕟'), key: m.key, }})
           json = asup[Math.floor(Math.random() * asup.length)]
           conn.sendVideo(m.chat, json.url, `Asupan Santuy\n\n${mess.done}`,m)
           break
case 'asupanukhty':
           res = await fetch('https://raw.githubusercontent.com/binjaicity/warga62/master/ukhty.json')
           asup = await res.json()
           conn.sendMessage(m.chat, {react: {text: ('🕟'), key: m.key, }})
           json = asup[Math.floor(Math.random() * asup.length)]
           conn.sendVideo(m.chat, json.url, `Asupan Ukhty\n\n${mess.done}`,m)
           break
}
}
handler.command = handler.help = ['asupan', 'asupanukhty', 'asupansantuy', 'asupangeayubi', 'asupanloli', 'asupanbocil']
handler.tags = ['asupan']
