...
async function yt(url, quality, type, bitrate, server = 'en68') {
	...
	var data = [],
		$ = cheerio.load(json.result)
	$('tbody > tr').each((index, e) => {
		var a = {
			res: $($(e).find('td')[0]).text(),
			size: $($(e).find('td')[1]).text()
		}
		data.push(a)
	});
	var list
	switch (type) {
		case 'mp4':
			list =
				Object.fromEntries(data.filter(v => !/\.3gp|.mp3/.test(v.res)).map(v => {
					return [v.res.match(/.*?(?=\()/)[0].trim(), v.size]
				}))
			break
		case 'mp3':
			list = {
				'128kbps': data.filter(v => !/\.3gp|.mp4/.test(v.res)).map(v => {
					return v.size
				}).pop()
			}
			break
	...
	var resUrl = /<a.+?href="(.+?)"/.exec(json2.result)[1]
	return {
...
module.exports = {
	 ...
	 * @param {String} server (avaiable: `id4`, `en60`, `en61`, `en68`)
	 */
	ytv(url, resol = '480p', server = 'en154') {
